...
class stock_move_analysis(models.Model):
    ...
    categ_id = fields.Many2one('product.category',string='Category Product',readonly=True)
    ...
    location_src_id= fields.Many2one('stock.location', 'Source Location', readonly=True)
    ...
    standard_price= fields.Float(digits_compute=dp.get_precision('Account'), string='Standard Price',readonly=True)
    total= fields.Float(digits_compute=dp.get_precision('Account'), string='Total',readonly=True)
    _order = 'date asc'
