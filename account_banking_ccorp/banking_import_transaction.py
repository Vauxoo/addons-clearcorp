...
class bankImportTransaction(osv.Model):
    ...
    """
    _name = 'account.banking.ccorp.bank.import.transaction'
    ...
    def _match_debit_order(self, cr, uid, trans, log, context=None):
        def is_zero(total):
        ...
        order_ids = payment_order_obj.search(
            cr, uid, [('state', '=', 'open'),
                      ('date_done', '<=', str2date(trans.execution_date,'%Y-%m-%d')),
                      ],
        ...
        candidates = [x for x in orders if
                      ...
                      x.line_ids and x.line_ids[0].move_line_id]
        if len(candidates) > 0:
            # retrieve the common account_id, if any
            account_id = False
            for line in candidates[0].line_ids[0].move_line_id.move_id.line_id:
    ...
    def _match_storno(self, cr, uid, trans, log, context=None):
        payment_line_obj = self.pool.get('payment.line')
        line_ids = payment_line_obj.search(
            cr, uid, [('order_id.state', 'in', ['open', 'done']),#['sent', 'done']
    ...
    def _match_move(self, cr, uid, trans, move_lines, context=None):
        digits = dp.get_precision('Account')(cr)[1]
        candidates = []
        secondary_candidates = []
        for move_line in move_lines:
            if move_line.ref == trans.reference:
                #Check if the journal currency corresponds to the company's currency
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        """
        res = super(bankImportTransaction, self).create(cr, uid, vals, context)
        if res and not context.get('transaction_no_duplicate_search'):
            me = self.browse(cr, uid, res, context)
            search_vals = [(key, '=', me[key]) for key in self.signal_duplicate_keys]
            ...
            for trans in self.browse(cr, uid, ids, context=context):
                if self.pool.get('res.currency').is_zero(cr, uid,
            ...
            if len(dupes) < 1:
                raise osv.except_osv(
                                     ...
                                     _('Cannot check for duplicate. '
                                       'I can\'t find myself.'))
            if len(dupes) > 1:
                self.write(cr, uid, res, {'duplicate': True}, context=context)
        return res
    ...
    def match(self, cr, uid, ids, context=None):
        ...
        while i < max_trans:
            move_info = False
            # Look for any available injected transaction
            if injected:
                # Force FIFO behavior
                transaction = injected.pop(0)
            else:
                transaction = transactions[i]
            # Validate statement's state
            ...
            if not move_info:
                ...
                move_info = self._match_move(cr, uid, transaction, transit_move_lines, context=context)
            ...
            company = company_obj.browse(cr, uid, transaction.company_id.id, context)
            ...
            if move_line_ids:
                ...
                move_lines = []
            # REPETITIVE CODE MUST BE CHECKED
            #BEGIN
            ...
            if transaction.type != bt.BANK_COSTS and transaction.provision_costs:
                ...
                cost_id = self.copy(
                    ...
                    dict(
                        ...
                        parent_id = transaction.id), context)
                ...
                dict(
                     transferred_amount = transaction.transferred_amount - transaction.provision_costs,
            ...
            if transaction.type == bt.DIRECT_DEBIT:
                move_info = self._match_debit_order(cr, uid, transaction, context)
            if transaction.type == bt.STORNO:
                move_info = self._match_storno(cr, uid, transaction, context)# TODO: Needs Review match not working
            # Allow inclusion of generated bank invoices
            if transaction.type == bt.BANK_COSTS:
                lines = self._match_costs(cr, uid, transaction, period_id, account_info)
                ...
                partner_banks = get_bank_accounts(self.pool, cr, uid,
                                                  transaction.remote_account, fail=True)
                if partner_banks:
                    partner_ids = [x.partner_id.id for x in partner_banks]
                elif transaction.remote_owner:
                    country_id = get_country_id(self.pool, cr, uid,
                                                transaction, context=context)
                    partner_id = get_partner(self.pool, cr, uid,
                    ...
                    if partner_id:
                        ...
                        if transaction.remote_account:
                            ...
                            partner_banks = partner_bank_obj.browse(cr, uid, [partner_bank_id])
            ...
            if move_info:
                ...
                self_values['move_line_ids'] = [(6, 0, move_info.get('move_line_ids') or [])]#Why 6,0?
            ...
            if not transaction.statement_line_id:
                values.update(dict(
                        name = (transaction.statement and transaction.statement +
                ...
                statement_line_obj.write(cr, uid,
                                         transaction.statement_line_id.id,
                                         values, context)
        ...
        if payment_lines:
            ...
            if order_ids:
                ...
                payment_order_obj.set_done(cr, uid, order_ids,{'state': 'done'})
                wf_service = netsvc.LocalService('workflow')
                for id in order_ids:
...
#>>>>>>> MERGE-SOURCE
    ...
    _columns = {
        ...
        'parent_id': fields.many2one(
            'account.banking.ccorp.bank.import.transaction', 'Split off from this transaction'),
        # match fields
...
class account_bank_statement_line(osv.Model):
    ...
    _columns = {
        'import_transaction_id': fields.many2one(
            'account.banking.ccorp.bank.import.transaction', 
    ...
    def create_instant_transaction(
        ...
        """
        import_transaction_pool = self.pool.get('account.banking.ccorp.bank.import.transaction')
    ...
    def split_off(self, cr, uid, ids, amount, context=None):
        ...
        transaction_pool = self.pool.get('account.banking.ccorp.bank.import.transaction')
