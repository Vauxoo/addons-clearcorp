...
&sensor=false&zoom=8&size=298x298""" % werkzeug.url_quote_plus(
    ...
    def get_contactus_response(self, values, kwargs):
        ...
        return request.website.render(kwargs.get("view_callback",
                                      "website_crm.contactus_thanks"), values)

    @http.route(['/website_contact_form/contact'], type='http', auth="public",
                website=True)
    def contact(self, **kwargs):
        ...
        _REQUIRED = ['contact_name', 'email_from']
        ...
        values['medium_id'] =\
            request.registry['ir.model.data'].xmlid_to_res_id(
        ...
        # if kwarg.name is empty, it's an error, we cannot
        # copy the contact_name
        ...
        if kwargs.get("show_info"):
            ...
            post_description.append("%s: %s" % ("IP",
                                    environ.get("REMOTE_ADDR")))
            post_description.append("%s: %s" % ("USER_AGENT",
                                    environ.get("HTTP_USER_AGENT")))
            post_description.append("%s: %s" % ("ACCEPT_LANGUAGE",
                                    environ.get(
                                                "HTTP_ACCEPT_LANGUAGE")))
            post_description.append("%s: %s" % ("REFERER",
