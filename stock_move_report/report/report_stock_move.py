...
############################Report Functions################################
    ...
    def get_destination_location_ids(self, data):
        return self._get_info(data,'destination_location_ids', 'stock.location')
    ...
    def get_search_criteria(self,data,product_id):
        ...
        if partner_list:
            ...
            partner_ids = self.pool.get('res.partner').search(self.cr, self.uid, [])
        
        source_location_list = self.get_source_location_ids(data)#Filter Source location
        if source_location_list:
            for source_location in source_location_list:
                source_location_ids.append(source_location.id)
        else:
            source_location_ids = self.pool.get('stock.location').search(self.cr, self.uid, [])
        
        destination_location_list = self.get_destination_location_ids(data)#Filter Destination location
        if destination_location_list:
            ...
            destination_location_ids = self.pool.get('stock.location').search(self.cr, self.uid, [])
            
        picking_type_list = self.get_picking_type_ids(data)#Filter Picking Type
        if picking_type_list:
            for picking_type in picking_type_list:
                picking_type_ids.append(picking_type.id)
        else:
            picking_type_ids = self.pool.get('stock.picking.type').search(self.cr, self.uid, [])
...
###############Display Data###################
    ...
    def set_opening_quantity(self, data, product_id, location_id):
        date_from = self.get_date_from(data)
        opening_quantity = self.get_opening_quantity(data, product_id, location_id)
    ...
    def get_opening_cost(self, data,product_id,location_id,opening_quantity):
        opening_cost=0.0
        quantity=0.0
        cost_quant=0.0
        stock_move_obj = self.pool.get('stock.move')
