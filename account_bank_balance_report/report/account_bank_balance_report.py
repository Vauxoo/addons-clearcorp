...
class Parser(accountReportbase):
    ...
    def display_account_name(self, data, account_id):
        ...
        if bank_account.default_credit_account_id and\
            ...
            if bank_account.default_credit_account_id.id ==\
                ...
                if bank_account.default_credit_account_id:
                    if bank_account.default_credit_account_id.id == account_id:
                        str_name = _('Default credit account: ') +\
                            bank_account.default_credit_account_id.code +\
                            ' - ' +\
                            bank_account.default_credit_account_id.name + ' - '
                        if bank_account.default_credit_account_id.currency_id:
                            cred = bank_account.default_credit_account_id
                    ...
                    if bank_account.default_debit_account_id.id == account_id:
                        str_name = _('Default debit account: ') +\
                            bank_account.default_debit_account_id.code +\
                            ' - ' +\
                            bank_account.default_debit_account_id.name + ' - '

                        if bank_account.default_debit_account_id.currency_id:
                            deb = bank_account.default_debit_account_id
                            str_name += deb.currency_id.name
                        else:
                            str_name += user.company_id.currency_id.name
        else:
            if bank_account.default_credit_account_id:
    ...
    def get_currency_company(self):
        return self.pool.get('res.users').browse(
            self.cr, self.uid, [self.uid])[0].company_id.currency_id
    ...
    def update_cumul_balance(self, line):
        ...
        if not line.currency_id or line.currency_id.id ==\
                self.get_currency_company():
            cumul_balance = self.localcontext['storage']['cumul_balance'] +\
    ...
    def return_balance_account(self, data, account_id):
        ...
        if account.currency_id == currency_company or not account.currency_id:
            ...
            # initialize cum_balance
            dict_update = {
                'cumul_balance': balance[account_id]['foreign_balance']
            }
            self.localcontext['storage'].update(dict_update)
    ...
    def process_move_lines(self, data):
        ...
        move_lines_ids = self.pool.get('account.move.line').search(
            self.cr, self.uid, [('account_id', 'in', account_ids),
                                ('state', '=', 'valid'),
                                ('period_id', '=', period.id)])
        move_lines = self.pool.get('account.move.line').browse(
            self.cr, self.uid, move_lines_ids)
        return move_lines
    ...
    def get_initial_balance(self, data, account_id):
        ...
        previous_period = self.pool.get(
            'account.period').get_start_previous_period(
        ...
        if account.currency_id.id == currency_company or not\
            ...
            account_balance = library_obj.get_account_balance(
                ...
                start_period_id=previous_period, end_period_id=previous_period,
                filter_type='filter_period')
        else:
            account_balance = library_obj.get_account_balance(
                self.cr, self.uid, [account_id], ['foreign_balance'],
                initial_balance=True, fiscal_year_id=fiscal_year.id,
                start_period_id=previous_period, end_period_id=previous_period,
                filter_type='filter_period')
