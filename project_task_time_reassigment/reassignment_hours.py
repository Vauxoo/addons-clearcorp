...
class origin_task(models.Model):
    ...
    rh_origin_task = fields.Float('Remaining hours', related='origin_task_id.remaining_hours', readonly=True)
    ...
    def _compute_total_time_reassignment(self):
        for reassignment in self:
            if reassignment.inv_total_time_reassignment:
                reassignment.total_time_reassignment = reassignment.inv_total_time_reassignment
            else:
                for task in reassignment.origin_task_ids:
                    reassignment.total_time_reassignment = reassignment.total_time_reassignment + task.reassignment_hour
        return True

    def _inverse_total_time_reassignment(self):
        for reassignment in self:
            reassignment.inv_total_time_reassignment = reassignment.total_time_reassignment
        return True
            ...
            if not reassignment.target_task:
                for task in reassignment.origin_task_ids:
                    if task.reassignment_hour <= task.origin_task_id.remaining_hours:
                        task.origin_task_id.reassignment_hour = task.reassignment_hour * -1
                    else:
