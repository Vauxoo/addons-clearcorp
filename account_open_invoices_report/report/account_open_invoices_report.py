...
from openerp.addons.account_report_lib.account_report_base import \
    accountReportbase  # Library Base
...
class Parser(accountReportbase):
    ...
    def built_parameters(self, cr, uid, data):
        ...
        # if partner_ids is not false, it's means that at least a
        # partner has been selected in wizard. If it's false, we take in
        ...
        if partner_list:
            ...
            partner_ids = self.pool.get('res.partner').search(
                cr, uid, [], order='name ASC')
        
        return filter_type, filter_data, fiscalyear,\
            target_move, partner_ids, historic_strict
    ...
    def get_move_lines(self, cr, uid, data):
        ...
        filter_type, filter_data, fiscalyear,\
            target_move, partner_ids, historic_strict = self.built_parameters(
                cr, uid, data)
        account_ids = self.get_account_list(cr, uid, data)
        
        # If in wizard at least one partner is selected,
        # filter move lines by partner
        if partner_ids:
            move_lines = account_library.get_move_lines_partner(
                cr, 1, partner_ids, account_ids, filter_type=filter_type,
                filter_data=filter_data, fiscalyear=fiscalyear,
                target_move=target_move, unreconcile=True,
                historic_strict=historic_strict,
                order_by='date ASC')
        else:       
            # Return all move lines (With all partners)            
            move_lines = account_library.get_move_lines(
                cr, 1, partner_ids, account_ids, filter_type=filter_type,
                filter_data=filter_data, fiscalyear=fiscalyear,
                target_move=target_move, unreconcile=True,
                historic_strict=historic_strict, order_by='date ASC')
        return move_lines
        ...
        *result is a dictionary of dictionaries
        ...
        for currency, partners in result.iteritems():
            ...
            for partner, lines in partner.iteritems():
                ...
                for line in lines:
                    print line.name, line.ref ...
                    (lines is a list of move_lines)
        
        Also, return a list of partners sort by name. This is
        for show partners sort by name
    """
    def built_result(self, cr, uid, data):
        list_ids = []
        result = {}
        currency_obj = self.pool.get('res.users')
        currency_company = currency_obj.browse(
            cr, uid, [uid])[0].company_id.currency_id
        ...
        for line in move_lines:
            if not line.currency_id or \
                            line.currency_id.name == currency_company.name:
                ...
                if not line.partner_id:
                    if 'no_partner' not in result[currency_company.id].keys():
                        ...
                        list_ids.append(0)
                    result[currency_company.id][0].append(line)
                    # zero is key for 'no partner'

                else:
                    if line.partner_id.id not in result[
                            currency_company.id].keys():
                        result[currency_company.id][line.partner_id.id] = []
                        # ID list (without repeat id)
                        list_ids.append(line.partner_id.id)
                    result[currency_company.id][
                        line.partner_id.id].append(line)
        ...
        partner_ids_order = self.pool.get('res.partner').search(
            cr, uid, [('id', 'in', list_ids)], order='name ASC')

        return result, partner_ids_order
   
    # 4. Get amounts for each move_line
    # ========================================================================
    # Return a dictionary, where the key is move_line id and it
    # contains another dictionary with 6 values: Invoices, voucher,
    # credit, debit, manual and balance.
    # ========================================================================
    def get_amounts_move_line(self, cr, uid, move_lines, currency_id):
        
        company_currency = self.pool.get('res.users').browse(
            cr, uid, [uid])[0].company_id.currency_id.id
        ...
        for line in move_lines:       
            ...
            invoice_id = invoice_obj.search(
                cr, uid, [('move_id', '=', line.move_id.id)], limit=1)
            ...
            if invoices:
                for invoice in invoices:
                    ...
                    elif invoice.type == 'in_refund' or \
    ...
    """
    def compute_inicial_balance(self, partner_id, currency_id, data):
        result = {}
        params = []
        where_clause = ''
        ...
        # if filter_type == period, get all periods before
        ...
        if filter_type == 'filter_period':
            ...
            if currency_id == user.company_id.currency_id.id:
                where_clause += "AND (" \
                                "currency_id is NULL OR currency_id = %s) "
            ...
            sql = ("SELECT partner_id as partner," + sum_str +
                   "as initial_balance "
        ...
        # if filter_type == date, get take in account date start selected
            ...
            # Add account and reconcile
            where_clause = "date < %s " + " AND reconcile_id is NULL " +\
                           "AND account_id in %s "
            params.append(date)
            params.append(tuple(account_ids))
        ...
        # if it exists lines, built a new dictionary, where the key is
        # the partner with initial_balance
        res = self.cursor.dictfetchall()
        
        if res:
            for vals_dict in res:
                result[vals_dict['partner']] = vals_dict['initial_balance']
                # set this result as the first amount of initial balance
                self.localcontext[
                    'cumul_balance'][vals_dict['partner']] = \
                    result[vals_dict['partner']]
        else:
    ...
    def get_check_open_balance(self, data):
        result = {}       
        active_curency_ids = self.pool.get('res.currency').search(
            self.cr, self.uid, [('active', '=', True)], context=None)
                ...
                if main_dict:
                    if currency in main_dict.keys():
                        if partner not in main_dict[currency].keys():
                            res = self.compute_inicial_balance(
                                self.cr, self.uid, partner, currency, data)
    ...
    def compute_cum_balance(self, partner, res):
        ...
        cumul_balance = cumul_balance + res['invoice'] + res['c_d_notes'] + \
            res['payment'] + res['manual']
        self.localcontext['cumul_balance'][partner] = cumul_balance
        
        return cumul_balance
    
    # Return the total balance for each amount. Compute at
    ...
    def result_lines(self, move_lines, currency_id):
        amount_per_line = self.get_amounts_move_line(
            self.cr, self.uid, move_lines, currency_id)
    ...
    """@param partner: partner is an id (integer).
    It's comming for the odt template """
    ...
    def set_partners(self, cr, uid, data):
        ...
        # Partner
        partner_list = self.get_partner_ids(data)
        # if partner_ids is not false, it's means that at least a partner
        # has been selected in wizard. If it's false, we take in
        # account all partners
        if partner_list:
            for partner in partner_list:
                partner_ids.append(partner.id)
    ...
    def get_currency_name(self, currency_id):
        return self.pool.get('res.currency').browse(
            self.cr, self.uid, currency_id).name
    
    # Display partner if it has a open balances without open invoices
    def display_partner(self, vals_dict):
        display = False
       
        for currency, value in vals_dict.iteritems():
            # if any currency has a value, the partner will be print in report
            if value != 0:
                display = True
        return display
