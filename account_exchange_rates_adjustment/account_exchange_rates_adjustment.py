...
class AccountAccount(osv.osv):
    ...
    _columns = {
        'exchange_rate_adjustment': fields.boolean(
            'Exchange rate adjustment',
            help="Choose if the account needed an adjusted exchange rate"),
        'rate_adjustment': fields.selection([('primary','Primary'),
                                             ('secondary','Secondary')],
                                            "Rate Adjustment"),
    }
    ...
    def onchange_currency(self, cr, uid, ids, currency_id, context=None):
        ...
        if currency_id:
            currency_obj = self.pool.get('res.currency').browse(
...
class AccountMoveLine(osv.osv):
    ...
    def _amount_exchange_rate(self, cr, uid, ids, field_names,
                              args, context=None):
        """
           This function returns an amount of exchange rate base in the 
           debit/credit and amount_currency,
        ...
        for move_line in lines:
            ...
            if move_line.debit and move_line.debit > 0.00:
                ...
                res[move_line.id]['amount_exchange_rate'] = \
                    res_currency_obj.round(
                        cr, uid, move_line.currency_id, result) or result
                ...
                res[move_line.id]['amount_exchange_rate'] = \
                    res_currency_obj.round(
                        cr, uid, move_line.currency_id, result) or result
            res[move_line.id]['amount_base_import_exchange_rate'] = \
                move_line.currency_id.get_exchange_rate(company_currency,
                                                        move_line.date)[0]
        return res

    _columns = {
        'amount_exchange_rate': fields.function(
            _amount_exchange_rate, string='Amount of exchange rate',
            multi="residual", help="The amount of exchange rate."),
        'amount_base_import_exchange_rate': fields.function(
            _amount_exchange_rate, string='Base amount exchange rate',
            multi="residual", help="Exchange rate in the system."),
...
class AccountMove(osv.osv):
    ...
    def create_move_lines_reconcile(self, cr, uid, move, exchange_rate_date,
                                    context=None):
        ...
        account_reconcile_ids = account_account_obj.search(
            cr, uid, [('exchange_rate_adjustment', '=', True),
                       ('reconcile', '=', True)], context=context)
        periods_ids = account_period_obj.get_interval_period(cr, uid,
                          start_period=None, end_period=move.period_id,
                          fiscal_year=move.period_id.fiscalyear_id.id,
                          initial_balance=False)
        line_reconcile_ids = move_line_obj.search(
            cr, uid, [('date','<=',exchange_rate_date),
            ('currency_id','!=',None), ('period_id','in',periods_ids),
            ('amount_currency','!=',0),
            ('account_id','in',account_reconcile_ids), ('adjustment','=',None),
            ('reconcile_id','=',None)], context=context)
        lines_reconcile = move_line_obj.browse(cr, uid, line_reconcile_ids,
                                               context=context)
        ...
        for line in lines_reconcile:
            ...
            line_difference = 0
            adjustment_amount = self.get_adjustment_amount(cr, uid, line.id,
                                                           context=context)
            ...
            exchange_amount = res_currency_obj._current_rate(
                cr, uid, [company_currency.id], exchange_rate_date, arg=None,
                context=copy_context)[company_currency.id]

            if line.credit != 0.00:
                line_difference = sign_amount_currency * \
                    line.amount_currency * exchange_amount - line.credit
            elif line.debit != 0.00:
                line_difference = sign_amount_currency * \
                    line.amount_currency * exchange_amount - line.debit
            ...
            if line.credit != 0 and \
                    exchange_amount > line.amount_exchange_rate or \
                    line.debit != 0 and \
                    exchange_amount < line.amount_exchange_rate:
                line_difference_adjustment = sign * line_difference + \
                adjustment_amount
            ...
            new_move_line_id = move_line_obj.create(cr, uid, move_line,
                                                    context=context)
            move_line_obj.reconcile_partial(cr, uid,
                                            [line.id, new_move_line_id],
                                            'auto', context)
            lines_created_ids.append(new_move_line_id)
            
        return lines_created_ids

    def create_move_lines_unreconcile(self, cr, uid, move,
                                      period, exchange_rate_date,
                                      context=None):
        res_currency_obj = self.pool.get('res.currency')
        move_line_obj = self.pool.get('account.move.line')
        account_account_obj = self.pool.get('account.account')
        ...
        account_unreconcile_ids = account_account_obj.search(
            cr, uid, [('exchange_rate_adjustment', '=', True), 
                      ('reconcile', '=', False), ('currency_id', '!=', None)],
            context=context)
        period_ids = account_period_obj.get_interval_period(
            cr, uid, start_period=None, end_period=period,
            fiscal_year=period.fiscalyear_id.id, initial_balance=False)

        for account_id in account_unreconcile_ids:
            res = library_obj.get_account_balance(
                cr, uid, [account_id], ['balance', 'foreign_balance'],
                fiscal_year_id = period.fiscalyear_id.id, state='posted',
                start_date = period.fiscalyear_id.date_start,
                end_date = exchange_rate_date, period_ids = period_ids)
            foreign_balance = res[account_id]['foreign_balance']
            balance = res[account_id]['balance']

            #Search account
            account = account_account_obj.browse(cr, uid, account_id,
                                                 context=context)            
            #Check if account in line use primary or second rate. 
            ...
            account_difference = abs(foreign_balance) * \
                                    exchange_amount - abs(balance)
                        
            if account_difference > 0.0 and foreign_balance > 0.0 or \
                    account_difference < 0.0 and foreign_balance < 0.0:
                total_debit = abs(account_difference)
                total_credit = 0.0
            elif account_difference < 0.0 and foreign_balance > 0.0 or \
                    account_difference > 0.0 and foreign_balance < 0.0:
                total_debit = 0.0
    ...
    def create_balance_line(self, cr, uid, move, res_user, name,
                            context=None):
        move_line_obj = self.pool.get('account.move.line')
        amount = self.get_balance_amount(cr, uid, move.id, context=context)
        if amount > 0:
            company = res_user.company_id
            account_id = company.expense_currency_exchange_account_id.id
            if not account_id:
                raise osv.except_osv(
                    _('Error!'),
                    _('The company have not associated a expense account.\n'))
            credit = amount
            debit = 0.00
        else:
            company = res_user.company_id
            ...
            if not account_id:
                raise osv.except_osv(
                    _('Error!'),
                    _('The company have not associated a income account.\n'))
            credit = 0.00
            debit = amount * -1    
    ...
    def generate_adjustment_move(self, cr, uid, reference,
                                 journal, period, exchange_rate_date=False,
                                 context=None):
        res_user_obj = self.pool.get('res.users')
        res_user = res_user_obj.browse(cr, uid, uid, context=context)
        ...
        lines_reconcile_ids = self.create_move_lines_reconcile(
            cr, uid, move_created, exchange_rate_date, context=context)
        lines_unreconcile_ids = self.create_move_lines_unreconcile(
            cr, uid, move_created, period, exchange_rate_date, context=context)
        balance_line_id = self.create_balance_line(
            cr, uid, move_created, res_user, name, context=context)
