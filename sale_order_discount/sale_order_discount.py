...
class SaleOrder(osv.osv):
    ...
    _columns = {
        'amount_untaxed': fields.function(_amount_all, digits_compute= dp.get_precision('Account'), string='Subtotal With Discount',
            store = {
        ...
        'order_discount': fields.function(_amount_all, digits_compute= dp.get_precision('Account'), string='Discount (%)',
            store = {
                'sale.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line'], 10),
                'sale.order.line': (_get_order_ccorp, ['price_unit', 'tax_id', 'discount', 'product_uom_qty'], 10),
            },
    ...
    def button_dummy(self, cr, uid, ids, context={}):
        ...
        for sale in self.browse(cr, uid, ids, context=context):
            localcontext.update({'second_time': True})
