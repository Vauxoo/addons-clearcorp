...
class Report(models.Model):
    ...
    def get_html(self,ids, report_name, data=None):
        report = self._get_xls_report_from_name(report_name)
        if report:
            try:
                report_model_name = 'report.%s' % report_name
                particularreport_obj = self.env[report_model_name]
                return particularreport_obj.render_html(ids, data=data)
            except KeyError:
            ...
            return super(Report, self).get_html(ids, report_name, data=data)

    @api.model
    def get_xls(self,ids, report_name,html=None, data=None):
        """
        This method generates and returns xls version of a report.
        """
        report_obj = self.env['report']
        if html is None:
            html = report_obj.get_html(ids, report_name, data=data)
    ...
    @api.model
    def get_ods(self, ids, report_name,html=None, data=None):
        raise NotImplementedError

    def _get_xls_report_from_name(self,report_name):
        """
        ...
        idreport = report_obj.search(conditions)
        if idreport:
            return idreport[0]
        return None
