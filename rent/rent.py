...
class rent_building(osv.osv):
	...
	_columns = {
		...
		'building_estate'            : fields.many2one('rent.estate', 'estate',required=True),
...
class rent_local_floor(osv.osv):
	...
	_columns = {
		...
		'local_floor_floor'    : fields.many2one('rent.floor','Level',help='Represents the floor on witch its located the local',required=True),
...
class rent_floor_parking(osv.osv):
	...
	_columns = {
		...
		'parking_floor'           : fields.many2one('rent.floor','# Floor',required=True),
...
class rent_rent(osv.osv):
	...
	def register_historic(self,cr,uid,obj_rent):
		...
		if obj_rent:
			...
			if obj_rent.rent_related_real == 'local':
				vals['anual_value_local'] = obj_rent.rent_rent_local.id
			if not is_registrated:
				vals['rent_historic'] = [(0,0,{'anual_value_rent':obj_rent.id,'anual_value_value':years_val,'anual_value_prev_value' : prev_value,'anual_value_rate' : obj_rent.rent_rise, 'anual_value_date' : current_date, 'anual_value_type' : 'rent', 'anual_value_local':vals['anual_value_local']})]
			else:
	...
	def register_main_historic(self,cr,uid,obj_rent):
		...
		if obj_rent:
			...
			if obj_rent.rent_related_real == 'local':
				vals['anual_value_local'] = obj_rent.rent_rent_local.id
			if not is_registrated:
				vals['rent_main_historic_ids'] = [(0,0,{'anual_value_rent':obj_rent.id,'anual_value_value':years_val,'anual_value_prev_value' : prev_value,'anual_value_rate' : rise, 'anual_value_date' : current_date, 'anual_value_type' : 'main','anual_value_local':vals['anual_value_local']})]
			else:
	...
	_columns = {
		...
		'rent_rent_client'      : fields.many2one('res.partner','Client', required=True, states={'active':[('readonly',True)], 'finished':[('readonly',True)]}),
